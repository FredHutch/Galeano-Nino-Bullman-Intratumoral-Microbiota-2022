...
if __name__ == "__main__":
    ...
    for n in range(0,len(validate_csv_file_nova)):
        UMI_bac_list = UMI_bac_list+read_and_mkdic(path_nova + '/' + validate_csv_file_nova[n]+'.nova.filtered_matrix.validate.csv', 'sample_'+sample_name_nova[n])
    
    for n in range(0,len(validate_csv_file_mi)):
        UMI_bac_list = UMI_bac_list+read_and_mkdic(path_mi + '/' + validate_csv_file_mi[n]+'.mi.filtered_matrix.validate.csv', 'sample_'+sample_name_mi[n])
